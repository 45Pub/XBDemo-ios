...
- (BOOL)activate:(XMPPStream *)aXmppStream
{
	...
	if ([super activate:aXmppStream])
	{
		...
		#endif
		_rosterTracker = [[XMPPIDTracker alloc]initWithDispatchQueue:moduleQueue];
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
- (NSString *)addUserWithJidStr:(NSString *)jidStr nickname:(NSString *)optionalName subscribeToPresence:(BOOL)subscribe
{
	if (jidStr.length <= 0)
		return nil;
...
- (void)setNickname:(NSString *)nickname ifExsitUserJidstr:(NSString *)jidstr
{
	if ([xmppRosterStorage userExistsWithJID:[XMPPJID jidWithString:jidstr] xmppStream:nil]) {
		[self setNickname:nickname forUser:[XMPPJID	 jidWithString:jidstr]];
	}
}
...
- (BOOL)xmppStream:(XMPPStream *)sender didReceiveIQ:(XMPPIQ *)iq
{
	...
	if (query)
	{
		NSString *eleid = [iq elementID];
		if (eleid) {
			[_rosterTracker invokeForID:eleid withObject:iq];
		}
...
- (void)handleMyIQ:(XMPPIQ *)iq withInfo:(XMPPBasicTrackingInfo *)trackerInfo
{
//	XMPPLogVerbose(@"%@===%@", iq, trackerInfo);
	XMPPLogVerbose(@"%@==", trackerInfo.elementID);
	[multicastDelegate xmppRoster:self didTrackedID:trackerInfo.elementID iq:iq];
}

- (void)addTrackerID:(NSString *)elementID timeout:(NSInteger)timeout
{
	dispatch_block_t block = ^{ @autoreleasepool {
		
		[_rosterTracker addID:elementID
					   target:self
					 selector:@selector(handleMyIQ:withInfo:)
					  timeout:timeout];
	}};
