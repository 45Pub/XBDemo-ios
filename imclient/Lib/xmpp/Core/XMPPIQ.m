...
- (BOOL)errorWithRosterShouldAuth
{
	NSXMLElement *error = [self elementForName:@"error"];
	NSString *errorcode = [error attributeStringValueForName:@"code"];
	if(errorcode != nil && [errorcode isEqualToString:@"407"])
		return YES;
	return NO;
}

- (BOOL)errorWithRosterDenyAddFriend
{
	NSXMLElement *error = [self elementForName:@"error"];
	NSString *errorcode = [error attributeStringValueForName:@"code"];
	if(errorcode != nil && [errorcode isEqualToString:@"405"])
		return YES;
	return NO;
}
